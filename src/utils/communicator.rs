...
{
    fn handle_action(
        &self, action: changer::Action<Key, Value>
    ...
    Writer: Storage<Key, Value>
{
    pub fn new(writer: Writer, drop: bool) -> Self {
        ...
        let data = values.into_iter().map(|v|(v.get_key(), v)).collect::<HashMap<_, _>>();
        let (changer_sender, changer_reciver) = mpsc::channel::<changer::Action<Key, Value>>(5);
        let (data_sender, _) = watch::channel(data.clone());

        Self { 
            data, writer, data_broadcast: data_sender, processing_responses: vec![],
    ...
    fn setup_writer(writer: &Writer, drop: bool) -> Vec<Value> {
        ...
        let mut promise = ImmediateValuePromise::new(async move {
            Ok(future.await.unwrap())
        });
        loop {
            match promise.poll_state() {
                ImmediateValueState::Success(vec) => return vec.to_vec(),
    ...
    fn consume_finished(&mut self) {
        let loading_responses = self.processing_responses.drain(..)
    ...
    fn update_data(&mut self, response: &Response<Key, Value>) {
        if let Response::Worked(action) = response {
            self.data_broadcast.send_modify(|data| {
                match action {
                    changer::ActionType::Set(value) 
                    | changer::ActionType::Update(value) => {
                        println!("inserted stuff");
                        data.insert(value.get_key(), value.clone());
                    },
                    changer::ActionType::SetMany(values)
                    | changer::ActionType::UpdateMany(values) => {
