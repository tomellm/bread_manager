...
pub struct ExpenseRecordFullView {
    record: ExpenseRecord,
}

impl ExpenseRecordFullView {
    pub fn new(record: ExpenseRecord) -> Self {
        Self { record }
    }
...
impl Widget for &mut ExpenseRecordFullView {
    fn ui(self, ui: &mut egui::Ui) -> egui::Response {
        ui.vertical_centered(|ui| {
            Grid::new(self.uuid()).spacing([10., 10.]).show(ui, |ui| {
                ...
                ui.heading("Amount:");
                ui.heading(self.formatted_amount());
                ui.end_row();
...
impl Deref for ExpenseRecordFullView {
    type Target = ExpenseRecord;
